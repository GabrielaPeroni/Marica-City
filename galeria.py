...
def show_content(main_frame, items,  app):
    ...
    for item in items:
        ...
        button_details = CTkButton(master=frame, text="Ver mais", command=lambda p=item: show_gallery(main_frame, p, items,  app), fg_color="red")
...
def show_gallery(main_frame, place, all_items, app):
    ...
    back_button = CTkButton(master=main_frame, text="Voltar", command=lambda: show_content(main_frame, last_clicked_category,  app), fg_color="red")
    ...
    label_name = CTkLabel(master=main_frame, text=place["name"], font=("Arial", 23, "bold"))
